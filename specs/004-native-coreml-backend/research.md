...
dedicated PR to fix — the same class of bug this project had *already* reproduced once from scratch
...
002-transcription-progress's planning). No FluidAudio *code* is reused or vendored — para is Rust,
...
execution path because ONNX Runtime's CoreML *execution provider* measured zero speedup
...
numbers are about *their* Swift application, not a guarantee for para's independent Rust
