...
- [x] T015 Peak memory, long recording (~25.7 min), default model, via the real CLI: **211-214MB** — a >90% reduction from the ~2.7GB post-003 baseline, exceeding SC-001's ≥70% target by a wide margin, and beating VoiceInk's own reported ~500MB _incremental_ figure on a cold start
