...
| `--device <auto\|coreml\|cpu>`   | No       | `auto`                             | Overrides execution-provider selection; `auto` and `cpu` both run on the CPU execution provider everywhere, including darwin/aarch64 — CoreML measured zero speedup for this model family and is only used when explicitly requested via `--device coreml` (Constitution Principle VI, amended 2026-07-14; research.md §15). Not a spec.md requirement — a plan-level addition (research.md), documented here so it's part of the frozen contract rather than an undocumented assumption. |
...
- **stdout**: contains _only_ the transcript in the requested format. Nothing else is ever written
