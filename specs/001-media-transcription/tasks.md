...
- [ ] T010 [P] Define the static model registry (3 verified-real models: `parakeet-tdt-0.6b-v3` default TDT, `parakeet-tdt-0.6b-v2` TDT, `parakeet-ctc-0.6b` CTC — research.md §3) in `src/model/registry.rs`. Each entry's `files` list is its encoder + vocab.txt + (TDT only) decoder-joint graph — the mel preprocessor is fetched by `build.rs`, not part of this list (research.md §10; data-model.md). Download each model's real files first and compute actual SHA256 checksums — never placeholder them (Constitution Principle V). **Structure and real repo/file names are in place; checksums are still `None` pending the actual (multi-GB) downloads — not yet checked off.**
