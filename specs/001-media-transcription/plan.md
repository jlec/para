...
| VI. Apple Silicon First-Class     | `Device::Auto` defaults to CPU on all targets, including darwin/aarch64 — CoreML measured zero speedup over CPU for this model family and required a stability workaround for a real ONNX Runtime crash with these models' external-data storage. CoreML remains available via explicit `--device coreml`, with its compiled `.mlmodelc` artifacts cached across runs (`with_model_cache_dir`), unchanged from before.                             | PASS — Constitution amended to v2.0.0 (2026-07-14) after this was measured, not assumed (research.md §13-15): first, that `ort`'s `coreml` feature had never actually been enabled at all (§13); then, once enabled and made stable (`with_static_input_shapes(true)`), that it performs identically to plain CPU on real end-to-end timing (§15) |
