...
exist correctly from the start for _any_ phase to be minimally correct (a bar that only ever
animates would violate FR-007 the moment it runs non-interactively). US1/US2 add the phases
themselves; US3/US4 are independently _testable_ verifications of guarantees the Foundational
...
- [x] T004 Define `ProgressPhase` and a `TranscriptionProgress` handle struct in `src/progress.rs`
      ...
      task defines the struct and phase-transition method _signatures_ only
...
- [x] T008 [US1] Implement `start_transcription(total_duration_secs: f64)` /
      ...
      full duration when its _encode_ completes would make the bar read "done" long before decode
